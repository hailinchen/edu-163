...
edu.tool.CookieUtil = {
  ...
  get: function(name) {
    ...
    if (cookieStart > -1) {
      ...
      cookieValue = encodeURIComponent(document.cookie.substring(cookieStart + cookieName.langth, cookieEnd));
    }
...
edu.tool.ajax = function ajax(obj) {
  ...
  // 请求参数格式化，encodeURIComponent 编码参数可以出现 &
...
edu.ui.getMainClass = function(obj) {
  ...
  edu.tool.ajax({
    ...
    success : function (data) {
      ...
      // _data.pagination.pageSize 每页显示的课程数
...
edu.app.login = (function () {
  ...
  edu.tool.EventUtil.addHandler(submitBtn, 'click', function (e) {
    // 阻止表单提交的默认行为，防止 xhr.status 为 0 的情况出现
    e.preventDefault();
  ...
  var pointAddClickEvent = (function () {
    ...
    edu.tool.EventUtil.addHandler(point, 'click', function (e) {
      if (e.target.tagName == 'I') {
        ...
        bannerImg[index].style.opacity = 'i';
    ...
    edu.tool.EventUtil.addHandler(pagesIndex, 'click', function (e) {
      if (e.target.tagName === 'LI') {
        ...
        if (/\d/.test(index)) {
          ...
          e.target.className = 'select_index';
        }
